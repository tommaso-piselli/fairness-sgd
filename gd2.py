    ...
    def __init__(self, G, device='cpu'):
        ...
        for node in G:
            ...
            if node_color > 40:
                ...
                self.node_dic[node] = "blue"
            else:
                self.color_list.append("red")
                self.node_dic[node] = "red"

        # self.edge_dic = {}
        # for e in self.G.edges:
        #     self.edge_dic[e] = []
        #     self.edge_dic[e].append(self.node_dic[e[0]])
        #     self.edge_dic[e].append(self.node_dic[e[1]])
    ...
    def evaluate(
        ...
        for q in qualities:
            ...
            if q == 'stress':
                ...
                if mode == 'original':
                    qualityMeasures[q] = Q.unfairness(pos, self.D, self.W, self.node_dic, None) # sample a None
                elif mode == 'best_scale':
                    s = utils.best_scale_stress(pos, self.D, self.W)
                    qualityMeasures[q] = Q.unfairness(pos*s, self.D, self.W, self.node_dic, None)
